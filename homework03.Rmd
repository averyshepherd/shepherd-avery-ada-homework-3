...
boot_lm <- function(d, model, conf.level=.95, reps=1000) {
  object = lm(as.formula(model), d)
  mod <- lm.boot(object, reps)
  f$beta0 <- t(samples(mod, "coef"))[,1]
  f$beta1 <- t(samples(mod, "coef"))[,2]
  
  glimpse(f)
}
boot_lm(d, model = "WeaningAge_d ~ Brain_Size_Species_Mean")
```
